...
user_role=agent and ole_version=6 after one of two active memberships was
...
production frontend remains ealtornet-web.vercel.app; backend remains
ealtornet-production.up.railway.app; email sender remains
...
edrooms=3 filtering reduced the map to one marker from one filtered result.
...
display_name or gency_name on each list item. Backend should add those
...
GET /api/v1/properties/ does not document an gency_id query param.
If the list endpoint gains gency_id, the owner listings hook can switch to
that canonical filter.

Closed in Phase K frontend Stream B commit 89f2530. Railway OpenAPI now
documents gency_id on GET /api/v1/properties/, and agency-owner My
...
company_name, and no gency_name on list rows. Do not implement frontend
...
backend-blocked. Frontend commit 89f2530 reads gent_count and
...
components, plus a table of ecent_changes showing entity type, action,
timestamp, and actor. Section is gated to admin role only. Types regenerated
from production OpenAPI. 	sc, lint, and uild gates passed clean.

DEF-L-MOD-001 - CLOSED (2026-05-30): Backend L.2 deployed and frontend L.3
implemented. pnpm gen:types confirmed gency_approved in ModerationStatus
enum and agency-approve/reject endpoints. Frontend changes:
- moderation.ts: added gency_approved constant, label "Admin review",
  badge variant "warning".
- Agent dashboard: updated pending_review label to "Agency review" and added

## Additional Deferred Items (merged from docs/DEFERRED.md)
...
## DEF-007 - Local Windows 
ext build spawn EPERM blocker
**Observed:** 
ext build can fail on Windows during page data collection with
spawn EPERM.
...
**Observed:** No Content-Security-Policy, COOP, or X-Frame-Options headers.
**Note:** Noted by Lighthouse Best Practices audit.
**Priority:** Medium.
**Phase:** F
**Fix:** Implement via 
ext.config.ts headers config.
