...
- Location hierarchy UI is wired to the public endpoints in Phase J. Live production data remains sparse under `DEF-I-LOC-001`: `/locations/` has one Lagos/Lekki/phase 1 row, `/locations/states` returns `["lagos"]`, `/locations/cities?state=lagos` returns `["lekki"]`, and `/locations/neighborhoods?state=lagos&city=lekki` returns `["phase 1"]`. Property search still filters by `location_id` once the cascade resolves to a concrete row.
